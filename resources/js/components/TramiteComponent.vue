            ...
            <div class="d-flex">
                ...
                <div class="flex-grow-1">
                    ...
                    <div class="d-flex align-items-center justify-content-between flex-wrap" :class="!group ? 'mt-2' : ''">
                        ...
                        <!--begin::User-->
                        <div class="mr-auto " v-bind:class="{'espace-checkbox-text' : !group && tramite.status && (tramite.status == 99 || tramite.status == 98) && !cartComponent && ['notary_titular', 'notary_substitute'].includes(user.role_name)}" v-bind:style="[ cartComponent ? { width : '60%' } : { width: '70%' } ]">
                            <!--begin::Name-->
                            <a v-on:click="goTo(tramite)" class="d-flex text-dark over-primary font-size-h5 font-weight-bold mr-3 flex-column" v-if="">
                                <strong v-if="!cartComponent && (different || !group)" class="text-uppercase text-truncate">{{ tramite.nombre_servicio }} {{tramite.nombre_servicio && (tramite.titulo && tramite.nombre_servicio.toLowerCase() != tramite.titulo.toLowerCase()) ? `- ${ (tramite.info && tramite.info.tipoTramite) || tramite.tramite || tramite.titulo | capitalize }` : '' }}</strong>
                        ...
                        <div class="my-lg-0 my-1 d-flex flex-column boton-lista-mobil">
                            ...
                            <div class="btn-group mt-2" v-if="tramite.info && !cartComponent && type != 2 && ![7,8].includes(tramite.status)">
